...
The `boost-attrs` extension allows using boost functionality without the problems related to inheritance that usually come with using [`hx-boost`](https://htmx.org/attributes/hx-boost/). This extension adds boost-specific versions of `hx-swap`, `hx-target` and `hx-select`.
